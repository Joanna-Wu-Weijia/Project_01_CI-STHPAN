...
from src.basics import set_device, default_device
...
parser.add_argument('--num_workers', type=int,
                    default=0 if sys.platform == 'darwin' else 10,
                    help='DataLoader workers (default 0 on macOS for stability; increase if needed)')
...
# Pretrained checkpoint (must match the pretrain run folder name)
parser.add_argument('--n_epochs_pretrain', type=int, default=100, help='epochs used in pretrain (folder name)')
parser.add_argument('--mask_ratio', type=float, default=0.4, help='mask ratio used in pretrain (folder name)')
parser.add_argument('--pretrained_model', type=str, default=None,
                    help='Full path to pretrained model.pth; if omitted, built from hyperparameters under ../finetune/saved_models/')
...
# Pretrained weights: same folder layout as patchtst_pretrain.py (../finetune/saved_models/{market}/pretrained/...)
if args.pretrained_model is None:
    save_pretrained_model = (
        'patchtst_pretrained_cw' + str(args.context_points) + '_patch' + str(args.patch_len)
    ...
    _pretrain_root = os.path.normpath(
        os.path.join(_repo, '..', 'finetune', 'saved_models', args.market, 'pretrained', save_pretrained_model, 'model0')
    )
    args.pretrained_model = os.path.join(_pretrain_root, 'model.pth')
print('pretrained checkpoint:', args.pretrained_model)
...
def test_func(market,weight_path):
    # get dataloader
    dls = get_dls(args)
    model = get_model(dls.vars, args, head_type='prediction').to(default_device(use_cuda=True))
    # get callbacks
    cbs = [RevInCB(dls.vars, denorm=True)] if args.revin else []
    cbs += [PatchCB(patch_len=args.patch_len, stride=args.stride)]
    learn = Learner(dls, model, 
