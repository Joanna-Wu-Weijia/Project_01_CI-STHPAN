...
class Dataset_Stock(Dataset):
    ...
    def __read_data__(self):
        ...
        print(
            f"#Dataset_Stock loading CSVs for split={split_names[self.set_type]} "
            f"(~{len(tickers)} tickers) — this can take several minutes…",
            flush=True,
        )
        ...
        for index, ticker in enumerate(tickers):
            ...
            if (index + 1) % 50 == 0 or index + 1 == len(tickers):
                print(
                    f"  … loaded {index + 1}/{len(tickers)} tickers ({split_names[self.set_type]})",
                    flush=True,
                )
