...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int, num_patch:int,
        ...
        # Apple MPS: BatchNorm backward is unreliable; prefer LayerNorm unless user overrode norm
        if norm == 'BatchNorm' and getattr(torch.backends, 'mps', None) is not None and torch.backends.mps.is_available():
            norm = 'LayerNorm'
        # Backbone (norm must be passed explicitly — was missing before, so encoder always used BatchNorm)
...
class HGAT(nn.Module):
    def __init__(self, d_in, d_model, dropout):
        ...
        self.bn4 = nn.Sequential(Transpose(1, 2, contiguous=True), nn.BatchNorm1d(d_model), Transpose(1, 2, contiguous=True))
...
class TSTEncoderLayer(nn.Module):
    def __init__(self, d_model, n_heads, d_ff=256, store_attn=False,
        ...
        if "batch" in norm.lower():
            self.norm_attn = nn.Sequential(Transpose(1, 2, contiguous=True), nn.BatchNorm1d(d_model), Transpose(1, 2, contiguous=True))
        else:
        ...
        if "batch" in norm.lower():
            self.norm_ffn = nn.Sequential(Transpose(1, 2, contiguous=True), nn.BatchNorm1d(d_model), Transpose(1, 2, contiguous=True))
        else:
